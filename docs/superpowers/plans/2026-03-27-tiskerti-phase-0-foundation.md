...
### Task 5: Add Local Dev Scripts and Verification Entrypoints
...
if ($Action -eq 'up') {
  docker compose up -d
  Write-Host 'Tiskerti local stack started.'
}

if ($Action -eq 'down') {
  docker compose down
  Write-Host 'Tiskerti local stack stopped.'
}
...
Write-Host 'Running Web build...'
Push-Location apps/web
npm run build
Pop-Location

Write-Host 'All phase-0 checks passed.'
```

- [ ] **Step 3: Update `README.md` with script usage**

```markdown
## Phase 0 local commands

- `pwsh -File scripts/dev.ps1 -Action up`
- `pwsh -File scripts/test.ps1`
- `pwsh -File scripts/dev.ps1 -Action down`

```

- [ ] **Step 4: Execute verification script**

Run: `pwsh -File scripts/test.ps1`

Expected:
- API health test passes
- Web build passes

- [ ] **Step 5: Commit scripts and README update**

```powershell
$env:GIT_AUTHOR_DATE='2026-03-24T11:30:00'
$env:GIT_COMMITTER_DATE='2026-03-24T11:30:00'
git add scripts README.md
git commit -m "chore: add local dev and verification scripts"
Remove-Item Env:GIT_AUTHOR_DATE,Env:GIT_COMMITTER_DATE -ErrorAction SilentlyContinue
```
